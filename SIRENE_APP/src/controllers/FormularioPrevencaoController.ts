...
import { FormularioPrevencaoService } from '../services/FormularioPrevencaoService';

export class FormularioPrevencaoController {
  static async criarFormulario(req: Request, res: Response): Promise<void> {
    try {
      const formulario = await FormularioPrevencaoService.criarFormulario(req.body);
      res.status(201).json(formulario);
    } catch (error) {
      res.status(400).json({ error: 'Erro ao criar formulário de prevenção', details: error });
    }
  }

  static async listarFormularios(req: Request, res: Response): Promise<void> {
    try {
      const formularios = await FormularioPrevencaoService.listarFormularios();
  ...
  static async buscarPorId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioPrevencaoService.buscarPorId(id);
      if (!formulario) {
  ...
  static async atualizarFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      const formulario = await FormularioPrevencaoService.atualizarFormulario(id, req.body);
      res.json(formulario);
    } catch (error) {
  ...
  static async removerFormulario(req: Request, res: Response): Promise<void> {
    try {
      ...
      await FormularioPrevencaoService.removerFormulario(id);
      res.status(204).send();
    } catch (error) {
